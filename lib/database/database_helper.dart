...
class DatabaseHelper {
  ...
  Future<User> getLogin(String user, String password) async {
    Database db = await instance.database;
    var res = await db.rawQuery("SELECT * FROM my_table WHERE username = '$user' "
  ...
  Future<double> getBalance(String username) async {
    Database db = await instance.database;
    var res = await db.rawQuery("SELECT * FROM my_table WHERE username = '$username'");
    if (res.length > 0) {
      return res.first['balance'];
    }    return null;
  }

  Future<double> deposit(double balance, String username) async {
    Database db = await instance.database;
    await db.rawQuery("UPDATE my_table SET balance = '$balance' WHERE username = '$username'");
    var res = await db.rawQuery("SELECT * FROM my_table WHERE username = '$username'");
    if (res.length > 0) {
      return res.first['balance'];
    }
